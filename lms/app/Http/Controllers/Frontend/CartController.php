...
class CartController extends Controller
{
    ...
    public function Payment(Request $request){
        ...
        if ($request->cash_delivery == 'stripe') {
            ...
            return $this->MidtransOrder($request);
    ...
    //     $params = [
    //         'transaction_details' => [
    //             'order_id' => $paymentId,  // Use the inserted Payment ID as the order_id
    ...
    public function MidtransOrder(Request $request) {
        ...
        // Create a new Payment Record first
        $paymentId = Payment::insertGetId($data);
        ...
        //         'course_title' => $cart->name, // Menggunakan nama kursus dari keranjang
        //         'price' => $cart->price, // Menggunakan harga dari keranjang
        //     ];
        // }
    
        $params = [
            'transaction_details' => [
                'order_id' => 'EOS' . $paymentId,
                'gross_amount' => $total_amount,
            ],
            'customer_details' => [
        ...
        // Menyimpan order untuk setiap item di keranjang
        foreach ($carts as $cart) {
            $order = new Order();
            $order->payment_id = $paymentId;
            $order->user_id = $user->id;
            $order->course_id = $cart->id; // Menggunakan ID kursus dari keranjang
            $order->instructor_id = $cart->options->instructor; // Menggunakan ID instruktur dari keranjang
            $order->course_title = $cart->name; // Menggunakan nama kursus dari keranjang
            $order->price = $cart->price; // Menggunakan harga dari keranjang
            $order->save();
        }
